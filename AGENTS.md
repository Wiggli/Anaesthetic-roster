...
- Pull requests targeting `main` run one `verify` job containing release consistency checks, deterministic regression tests, one production build, build-artifact verification and the browser smoke suite. Superseded runs for the same pull request may be cancelled automatically.
- The `verify` job uploads the exact tested `dist` directory as the short-lived `pages-dist` artifact. On a push to `main`, Supabase migration runs only after `verify` succeeds, and GitHub Pages deploys that same tested artifact after migration succeeds. Do not rebuild a second deploy artifact.
