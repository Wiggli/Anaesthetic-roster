...
- `package.json` is synchronised with `APP_VERSION` from 37.33 onward; pin package versions, commit the lockfile, and keep package and runtime versions aligned for each release.
...
- Pull requests targeting `main` run the test job. Supabase migration and GitHub Pages deployment happen only after a push to `main`: tests must pass, then Supabase migrations run, then GitHub Pages deploys. Do not bypass the test or migration dependency.
