...
- The production application remains a static PWA at the same Pages URL. Since version 37.33, Vite builds `dist` from the HTML, an isolated React/TypeScript launch region, Tailwind v4, and the existing global application scripts. Migrate stable regions incrementally; preserve the global roster/backend contracts until each region has been reviewed and tested.
...
- Database migrations are forward-only, timestamped files stored in `supabase/migrations/` and named `YYYYMMDDHHMMSS_description.sql`. An already deployed migration must never be edited, renamed, moved, or reused. Statements should be safely repeatable where practical. Every schema upgrade must update `app_schema_version` atomically to the exact matching version.
...
- `release.json` is the source of truth for the current deployable release number and release metadata. `APP_VERSION` in `app-core.js`, `RELEASE_HISTORY[0].version` in `app-ui.js`, package metadata, service-worker cache names and production cache-busting references must match it.
- For a user-visible release, edit `release.json` with the new monotonically increasing version, date, title, summary and actual changes, then run `npm run release`. The release script prepends the new history entry when required and synchronises the runtime/package/PWA version references. Do not hand-edit the same version through multiple files unless repairing the release tooling itself.
- `npm run verify:release` is the canonical consistency check and is part of `npm test`. Tests that only need the current version must derive it from `release.json`; do not hard-code the newest version into multiple test files.
- Every user-visible release gets one new newest-first release entry containing only that version's actual changes. Never merge, rewrite, collapse or delete older entries. The regression suite protects the frozen historical archive while allowing future entries to be prepended without editing a giant expected-version list.
- A documentation-only, test-only or developer-workflow change that does not alter the deployed application does not require a runtime version bump or release entry.
...
- Pin package versions, commit the lockfile, and keep package and runtime versions aligned through the release script.

## Deployment and pull-request rules

- Work on a non-`main` branch. Complete the intended narrow change and local/static checks before opening the pull request where practical, so GitHub Actions is not repeatedly triggered while the branch is still being assembled.
- Pull requests targeting `main` run one required `test` job containing release consistency checks, deterministic regression tests, one production build, build-artifact verification and the browser smoke suite. Superseded runs for the same pull request may be cancelled automatically.
- The required `test` job uploads the exact tested `dist` directory as the short-lived `pages-dist` artifact. On a push to `main`, Supabase migration runs only after `test` succeeds, and GitHub Pages deploys that same tested artifact after migration succeeds. Do not rebuild a second deploy artifact.
- Database deployment requires repository secrets `SUPABASE_ACCESS_TOKEN` and `SUPABASE_DB_PASSWORD`; never print or embed them. The fixed project ref is public configuration. The workflow initialises Supabase, verifies the checked-in `supabase/migrations/*.sql` history, links the project, and runs `supabase db push --linked --yes` before publishing.
- The explicit legacy asset allow-list is in `vite.config.mts`; generated JS/CSS and the injected custom worker are built alongside it. When adding a required legacy asset, add it to that list and the relevant HTML/manifest/worker references. Do not deploy tests, documentation, historical snapshots, repository metadata, migrations or secrets.
- Keep pinned CDN dependencies and GitHub Action major versions deliberate. Review upgrades for browser support, cache behaviour, supply-chain impact and workflow compatibility rather than accepting automated version churn blindly.
- When monitoring CI, inspect job-level status first and fetch individual step logs only for a failed or unexpectedly stalled job. Avoid repeated step-by-step polling that adds noise without changing the decision.
