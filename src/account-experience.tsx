...
function ProfileEditor({ model }: { model: ProfileExperience }) {
  ...
  return <motion.section
    ...
    className="tw:grid tw:gap-4 tw:rounded-3xl tw:border tw:border-black/8 tw:bg-[var(--card)] tw:p-4 tw:shadow-sm dark:tw:border-white/10"
    ...
    <div className="tw:flex tw:items-center tw:gap-3">
      ...
      <div className="tw:min-w-0">
        ...
        <div className="tw:mt-2 tw:flex tw:flex-wrap tw:gap-2">
          ...
          <button type="button" id="removeProfilePhoto" onClick={() => { setPhotoUrl(''); act('profile-photo-remove'); }} className={`tw:rounded-full tw:bg-rose-500/10 tw:px-3 tw:py-1.5 tw:text-xs tw:font-bold tw:text-rose-700 dark:tw:text-rose-200 ${model.photoUrl || model.pendingPhoto ? '' : 'tw:hidden'}`}>{model.pendingPhoto ? 'Cancel' : 'Remove'}</button>
        </div>
