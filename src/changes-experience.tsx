...
function EmptyRecord({ children }: { children: string }) {
  return <div className="tw:rounded-2xl tw:border tw:border-dashed tw:border-black/12 tw:bg-[var(--surface)]/70 tw:px-4 tw:py-5 tw:text-center tw:text-sm tw:text-[var(--muted)] tw:dark:border-white/14">{children}</div>;
}
  ...
  return <div className="tw:grid tw:gap-2">
    <AnimatePresence initial={false} mode="popLayout">
      {records.length ? records.map((record, index) => <motion.article
        ...
        className="tw:flex tw:items-center tw:justify-between tw:gap-3 tw:rounded-2xl tw:border tw:border-black/8 tw:bg-[var(--card)] tw:p-3.5 tw:shadow-sm tw:dark:border-white/10"
      >
        <div className="tw:min-w-0">
          <div className="tw:flex tw:flex-wrap tw:items-center tw:gap-2">
            ...
            <button
              ...
              className={`tw:rounded-full tw:px-2.5 tw:py-1 tw:text-[0.68rem] tw:font-bold tw:tracking-wide ${record.needsAllocation ? 'tw:bg-amber-400/15 tw:text-amber-800 tw:dark:text-amber-200' : record.kind === 'absence' ? 'tw:bg-rose-400/12 tw:text-rose-700 tw:dark:text-rose-200' : 'tw:bg-teal-400/12 tw:text-teal-700 tw:dark:text-teal-200'}`}
...
function AllocationList({ model }: { model: ChangesExperience }) {
  ...
  return <div className="tw:grid tw:gap-2">{model.allocations.map(row => <motion.label layout key={row.key} className="tw:grid tw:gap-3 tw:rounded-2xl tw:border tw:border-black/8 tw:bg-[var(--card)] tw:p-3.5 tw:sm:grid-cols-[1fr_minmax(12rem,0.8fr)] tw:sm:items-center tw:dark:border-white/10">
    ...
    <select
      ...
      className="tw:min-h-11 tw:w-full tw:rounded-xl tw:border tw:border-black/10 tw:bg-[var(--surface)] tw:px-3 tw:text-sm tw:font-semibold tw:dark:border-white/12"
    >
...
function RoleOverrideEditor({ model }: { model: RoleOverride }) {
  ...
  return <details className="tw:overflow-hidden tw:rounded-2xl tw:border tw:border-black/8 tw:bg-[var(--card)] tw:dark:border-white/10" open={model.open}>
    ...
    <div className="tw:grid tw:gap-3 tw:border-t tw:border-black/8 tw:px-4 tw:py-4 tw:dark:border-white/10">
      ...
      <div className="tw:grid tw:gap-2 tw:sm:grid-cols-2">{model.keys.map(row => <label key={row.key} className={`tw:grid tw:gap-1.5 ${row.fullWidth ? 'tw:sm:col-span-2' : ''}`}><span className="tw:text-xs tw:font-bold tw:text-[var(--muted)]">{row.label}</span><select data-night-role={row.key} value={model.assignments[row.key] || ''} onChange={event => dispatchAction({ action: 'role-select', key: row.key, value: event.target.value })} className="tw:min-h-11 tw:rounded-xl tw:border tw:border-black/10 tw:bg-[var(--surface)] tw:px-3 tw:text-sm tw:font-semibold tw:dark:border-white/12">{model.names.map(name => <option key={name.value} value={name.value}>{name.label}</option>)}</select></label>)}</div>
      {model.dirty && <><div className="tw:rounded-full tw:bg-amber-400/15 tw:px-3 tw:py-1.5 tw:text-xs tw:font-bold tw:text-amber-800 tw:dark:text-amber-200">Unsaved night-only change</div><label className="tw:grid tw:gap-1.5"><span className="tw:text-xs tw:font-bold tw:text-[var(--muted)]">Reason for the change</span><input id="nightRoleReason" defaultValue={model.reason} maxLength={120} placeholder="For example, agreed role arrangement" onInput={event => dispatchAction({ action: 'role-reason', value: event.currentTarget.value })} className="tw:min-h-11 tw:rounded-xl tw:border tw:border-black/10 tw:bg-[var(--surface)] tw:px-3 tw:text-sm tw:dark:border-white/12" /></label><button type="button" id="saveNightRolesBtn" className="primary wide" disabled={!model.canSave} onClick={() => dispatchAction({ action: 'role-save' })}>Save night-only change</button></>}
