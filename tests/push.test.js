...
assert.equal(release.title, 'Swipes stay responsive between screens');
assert.ok(release.changes.some(item => /pending destination is completed immediately/i.test(item)), 'release notes must explain interruptible settling');
assert.ok(release.changes.some(item => /gestures follow each other closely/i.test(item)), 'release notes must explain rapid-navigation reliability');
