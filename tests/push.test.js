...
assert.match(html, /push\.js\?v=37\.42/, 'push client must be versioned with the app');
...
assert.equal(release.version, '37.42');
assert.equal(release.title, 'Reliable swiping everywhere');
assert.ok(release.changes.some(item => /large dead zones.*ordinary cards.*Chat transcript/i.test(item)), 'release notes must explain the expanded swipe surfaces');
assert.ok(release.changes.some(item => /bottom selector now maps directly to the finger position/i.test(item)), 'release notes must explain direct full-track tab dragging');
