...
assert.match(html, /push\.js\?v=37\.43/, 'push client must be versioned with the app');
...
assert.equal(release.version, '37.43');
assert.equal(release.title, 'Solid continuous page flow');
assert.ok(release.changes.some(item => /fully opaque/i.test(item)), 'release notes must explain that staged pages stay opaque');
assert.ok(release.changes.some(item => /scale and transparency morph/i.test(item)), 'release notes must explain removal of the overlapping morph');
