...
assert.equal(release.title, 'Fluid screen transitions');
assert.ok(release.changes.some(item => /one continuous slide/i.test(item)), 'release notes must explain the seamless screen handoff');
assert.ok(release.changes.some(item => /Bottom-tab taps and long bottom-bar drags/i.test(item)), 'release notes must explain consistent navigation motion');
