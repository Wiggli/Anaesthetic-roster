...
assert.match(html, /push\.js\?v=37\.39/, 'push client must be versioned with the app');
...
assert.equal(release.version, '37.39');
assert.equal(release.title, 'A tab slider that follows your finger');
assert.ok(release.changes.some(item => /highlight tracks.*drag/i.test(item)), 'release notes must explain the draggable tab highlight');
assert.ok(release.changes.some(item => /Swipe right.*tab on the right/i.test(item)), 'release notes must explain the chosen direction');
