...
assert.ok(navigation.includes("let pendingSettle: { from: Destination; to: Destination | null } | null = null;"), 'page settling must remember a pending destination so it can be completed when interrupted');
assert.ok(navigation.includes("if (pendingSettle?.to && currentView === pendingSettle.from) finishContentSettle();"), 'a new touch during a committed settle must finish that destination instead of being dropped');
assert.doesNotMatch(navigation, /document\.querySelector\('main\.viewSwipeSettling'\)\) return;/, 'settling must no longer reject a new touch outright');
