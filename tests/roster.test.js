...
assert.ok(navigation.includes('const barPositionForClientX = (clientX: number) =>'), 'bottom-tab dragging must map the selector directly to the finger position');
assert.ok(navigation.includes('if (draggedPosition !== undefined) indicatorX.set(draggedPosition);'), 'bottom-tab dragging must track continuously across the full bar');
assert.ok(navigation.includes("nearestPositionIndex(reducedMotion && draggedPosition !== undefined ? draggedPosition : indicatorX.get())"), 'a long bottom-bar drag must settle on the nearest tab rather than only one neighbour');
assert.ok(navigation.includes("if (!destinations.includes(view) || (!inBar && !target.closest('main .view'))) return;"), 'Chat must remain eligible for safe content swipes');
assert.ok(navigation.includes("const blockedContentSelector = 'button,a,input,select,textarea,[role=\"button\"],[contenteditable=\"true\"],.chatComposer';"), 'only genuinely interactive content should block a page swipe');
assert.doesNotMatch(navigation, /blockedContentSelector[^\n]*(nightStatusRow|changesWorkflowTabs|dateNav|chatMessageViewport|chatConversationList|\[tabindex\])/, 'ordinary Night and Chat surfaces must not remain swipe dead zones');
