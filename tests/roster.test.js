...
assert.ok(navigation.includes('const step = start.bar ? Math.sign(dx) : -Math.sign(dx);'), 'content swipes must invert destination direction while tab-bar drags remain direct');
assert.ok(navigation.includes('current.style.transform = \`translate3d(\${dx}px,0,0)\`;'), 'the visible content page must track the finger during a drag');
assert.ok(navigation.includes('preview.style.transform = \`translate3d(\${dx + direction * width}px,0,0)\`;'), 'the adjacent page must enter from the correct side');
