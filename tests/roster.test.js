...
assert.match(workflow, /migrate:[\s\S]*needs: verify/, 'migration must depend on the complete verification job');
assert.match(workflow, /deploy:[\s\S]*needs: migrate/, 'deployment must depend on migration');
assert.match(workflow, /actions\/upload-artifact@v4[\s\S]*name: pages-dist[\s\S]*deploy:[\s\S]*actions\/download-artifact@v4[\s\S]*name: pages-dist/, 'deployment must use the exact dist artifact that passed the verification job');
...
assert.match(workflow, /verify:[\s\S]*@playwright\/test@1\.55\.0[\s\S]*playwright test/, 'the verification job must run a real-browser smoke suite before production migration');
assert.match(workflow, /migrate:[\s\S]*needs: verify/, 'production migration must wait for deterministic and browser verification');
