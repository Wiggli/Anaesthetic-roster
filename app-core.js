...
var currentPrivateProfile = null;
var profileFeatureAvailable = true;
...
function professionalName(name){var value=String(name||'').trim();return PROFESSIONAL_ROSTER_NAMES[value]||value}
function professionalNames(value){return String(value||'').split(' + ').map(professionalName).join(' + ')}
...
function withTimeout(promise,ms,message){
  return new Promise(function(resolve,reject){
    var settled=false,timer=setTimeout(function(){if(settled)return;settled=true;var error=new Error(message||'The request timed out.');error.code='TIMEOUT';reject(error)},ms);
...
function rememberAuthSession(session){currentAccessToken=session&&session.access_token?session.access_token:'';return session}
function authSessionNeedsRefresh(session){return !!(session&&(!session.access_token||!session.expires_at||Number(session.expires_at)*1000<=Date.now()+120000))}
async function refreshAuthSession(){var refreshed=await withTimeout(supa.auth.refreshSession(),10000,'The sign-in session could not be renewed.');if(refreshed.error)throw refreshed.error;if(!refreshed.data||!refreshed.data.session){var error=new Error('Your saved sign-in has expired.');error.code='AUTH_SESSION_MISSING';throw error}return rememberAuthSession(refreshed.data.session)}
...
function allocationLabel(key){return{first1:'First Part theatre · position 1',first2:'First Part theatre · position 2',second1:'Second Part theatre · position 1',second2:'Second Part theatre · position 2',pager:'Labour Ward / Pager',reliever:'Labour Ward / Reliever',seventh:'Seventh nurse'}[key]||'Additional allocation'}
function allocationBreak(key){if(key==='first1'||key==='first2')return'Second break';if(key==='second1'||key==='second2')return'First break';if(key==='pager')return'Labour Ward first part · Second break';if(key==='reliever')return'Labour Ward second part · First break';if(key==='seventh')return'Break coordinated as required';return'Break coordinated during the shift'}
...
function isMine(v){var n=myName();return n&&String(v).split(' + ').some(function(name){return canonicalNurseName(name)===canonicalNurseName(n)})}
function highlightNamed(rootId,selector){var n=myName();if(!n)return;Array.prototype.forEach.call(byId(rootId).querySelectorAll(selector),function(el){if(canonicalNurseName(el.textContent)===canonicalNurseName(n))el.classList.add('mine')})}
function renderMyName(){var el=byId('myNamePick'),selected=myName(),profileName=currentUserProfile&&currentUserProfile.display_name?currentUserProfile.display_name:'';if(!selected&&profileName){var match=TEAM.find(function(n){return canonicalNurseName(n)===canonicalNurseName(profileName)});if(match){selected=match;localStorage.setItem('anaes_my_name',match)}}var options='<option value="">Choose your name</option>'+TEAM.map(function(n){return '<option value="'+esc(n)+'">'+esc(professionalName(n))+'</option>'}).join('');el.innerHTML=options;el.value=TEAM.indexOf(selected)>-1?selected:''}
function fiveArrangementHtml(r){if(r.mode!=='5')return'';if(r.understaffedCount<5)return'<div class="arrangement"><h3>Additional cover required</h3><p class="time">'+esc(r.understaffedCount)+' nurses are currently recorded. The five-nurse allocation and break plan will appear after enough overtime cover is added and allocated.</p></div>';return'<div class="arrangement"><h3>Five-nurse arrangement</h3><p class="time">One nurse covers Labour Ward / Pager for the full night'+(r.automaticAbsence?' because '+esc(professionalName(r.automaticAbsence))+' is unavailable without replacement':'')+'.</p><div class="role rFull '+(isMine(r.fullLW)?'mine':'')+'"><div class="badge bFull">Full night</div><div><div class="name">'+esc(professionalName(r.fullLW))+'</div><div class="time">Labour Ward / Pager · 00:00–07:00 · Break coordinated when clinical cover allows</div></div></div></div>'}
...
function applyTheme(dark){var mode=dark?'dark':'light',background=dark?'#000000':'#f2f2f7';document.documentElement.setAttribute('data-theme',mode);document.documentElement.style.colorScheme=mode;document.documentElement.style.backgroundColor=background;document.body.classList.toggle('dark',dark);var colour=byId('themeColour');if(colour)colour.setAttribute('content',background);var button=byId('themeBtn');if(button){button.innerHTML=dark?'<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="4"></circle><path d="M12 2v2M12 20v2M4.9 4.9l1.4 1.4M17.7 17.7l1.4 1.4M2 12h2M20 12h2M4.9 19.1l1.4-1.4M17.7 6.3l1.4-1.4"></path></svg>':'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M20.5 14.2A8.5 8.5 0 0 1 9.8 3.5 8.5 8.5 0 1 0 20.5 14.2z"></path></svg>';button.setAttribute('aria-label',dark?'Use light mode':'Use dark mode')}updateAppearanceButtons()}
function themePreference(){var saved=null;try{saved=localStorage.getItem('anaes_theme')}catch(error){}if(['light','system','dark'].indexOf(saved)>=0)return saved;var legacy=null;try{legacy=localStorage.getItem('anaes_dark')}catch(error){}return legacy===null?'system':legacy==='1'?'dark':'light'}
function applyThemePreference(){var preference=themePreference(),dark=preference==='dark'||(preference==='system'&&window.matchMedia&&window.matchMedia('(prefers-color-scheme: dark)').matches);applyTheme(dark)}
function setThemePreference(preference){if(['light','system','dark'].indexOf(preference)<0)return;try{localStorage.setItem('anaes_theme',preference);localStorage.removeItem('anaes_dark')}catch(error){}applyThemePreference()}
function updateAppearanceButtons(){var preference=themePreference();Array.prototype.forEach.call(document.querySelectorAll('[data-theme-choice]'),function(button){var active=button.getAttribute('data-theme-choice')===preference;button.classList.toggle('active',active);button.setAttribute('aria-pressed',active?'true':'false')})}
function initTheme(){applyThemePreference();if(window.matchMedia){var query=window.matchMedia('(prefers-color-scheme: dark)');var listener=function(){if(themePreference()==='system')applyThemePreference()};if(query.addEventListener)query.addEventListener('change',listener);else if(query.addListener)query.addListener(listener)}}
function toggleTheme(){setThemePreference(document.body.classList.contains('dark')?'light':'dark')}
...
function renderAccounts(){
  var el=byId('adminAccountsExperience');if(!el)return;
  ...
  var detail={activeCount:active,inactiveCount:inactive,pending:accessRequests.map(function(r){return{userId:r.user_id,name:r.display_name,email:r.email,requested:shortTime(r.requested_at)}}),accounts:authorisedAccounts.map(function(a){return{email:a.email,name:a.display_name,role:a.user_role,active:!!a.active,current:a.email.toLowerCase()===currentUser.email.toLowerCase()}}),online:navigator.onLine&&(typeof forcedOfflineSession==='undefined'||!forcedOfflineSession)};
  updateAdminAttentionBadge();
  if(el.dataset.reactReady!=='true'){
    ...
    el.innerHTML='<div class="accountSummary"><div><b>'+active+' active member'+(active===1?'':'s')+'</b><div class="time">'+(pending?pending+' access request'+(pending===1?'':'s')+' waiting for review.':'New Google users can request access instead of being pre-added.')+'</div></div>'+(inactive?'<span class="pill">'+inactive+' inactive</span>':'')+'</div><div class="accountAdd"><label>Name<input id="accountName" autocomplete="off" autocapitalize="words" placeholder="Nurse name"></label><label>Government email<input id="accountEmail" type="email" inputmode="email" autocomplete="off" placeholder="Work email address"></label><button id="addAccountBtn">Add member</button></div>'+requests+authorisedAccounts.map(function(a){var self=a.email.toLowerCase()===currentUser.email.toLowerCase();return '<div class="accountRow"><div><b>'+esc(a.display_name)+'</b><div class="accountRole">'+esc(a.email)+' • '+esc(a.user_role)+(a.active?'':' • Inactive')+'</div></div><button class="mini '+(a.active?'':'soft')+'" data-account-toggle="'+esc(a.email)+'" '+(self?'disabled title="Your administrator account cannot be disabled here"':'')+'>'+(a.active?'Deactivate':'Activate')+'</button></div>'}).join('');
    var add=byId('addAccountBtn');if(add)add.onclick=addAuthorisedAccount;
  ...
  if(window.dispatchEvent&&typeof CustomEvent==='function')window.dispatchEvent(new CustomEvent('roster:admin-accounts',{detail:detail}));
}
...
function changeNight(step){var next=Math.max(0,Math.min(R.length-1,idx+step));if(next===idx)return;document.body.setAttribute('data-date-direction',step>0?'next':'previous');idx=next;render();setTimeout(function(){document.body.removeAttribute('data-date-direction')},360)}
function changeMyName(){var n=byId('myNamePick').value;if(n)localStorage.setItem('anaes_my_name',n);else localStorage.removeItem('anaes_my_name');render();toast(n?'Highlighting '+professionalName(n):'Name highlighting removed')}
function show(v){
  if(v==='admin'&&(!currentUserProfile||currentUserProfile.user_role!=='admin')){toast('Administrator access only');return}
  var previous=document.body.getAttribute('data-view'),target=byId(v),switching=previous!==v;
  if(!target)return;
  if(switching&&previous)viewScrollPositions[previous]=Math.max(0,Number(window.scrollY||0));
...
async function requestPasswordReset(){if(!supa){authMessage('Night Roster cannot connect right now. Check your internet connection and try again.',true);return}var email=byId('authEmail').value.trim().toLowerCase();if(!email||!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)){authMessage('Enter your roster email address first.',true);byId('authEmail').focus();return}var button=byId('forgotPasswordBtn');button.disabled=true;button.textContent='Sending link…';authMessage('Sending a secure reset link…');var result=await supa.auth.resetPasswordForEmail(email,{redirectTo:APP_URL});button.disabled=false;button.textContent='Forgot your password?';if(result.error){authMessage('The reset link could not be sent. Wait a moment and try again.',true);return}authMessage('If the account exists, a reset link is on its way. Check your inbox and junk folder.')}
async function signOutUser(){if(!confirm('Sign out of Night Roster?'))return;if(typeof realtimeGeneration!=='undefined')realtimeGeneration++;if(typeof startupAttempt!=='undefined')startupAttempt++;if(typeof forcedOfflineSession!=='undefined')forcedOfflineSession=false;if(typeof realtimeReconnectTimer!=='undefined'&&realtimeReconnectTimer){clearTimeout(realtimeReconnectTimer);realtimeReconnectTimer=null}if(typeof sharedSyncTimer!=='undefined'&&sharedSyncTimer){clearInterval(sharedSyncTimer);sharedSyncTimer=null}if(changesChannel){await supa.removeChannel(changesChannel);changesChannel=null}await supa.auth.signOut({scope:'local'});clearPrivateDeviceData();currentUser=null;currentAccessToken='';currentUserProfile=null;currentPrivateProfile=null;profileAvatarUrl='';nightChanges={};nightOvertime={};changeHistory={};overtimeHistory={};fiveCoverChoices={};setAuthMode('login');showAuth('You are signed out.')}
async function initApplication(){if(!supa){showAuth('Night Roster cannot connect right now. Check your internet connection and try again.',true);return}supa.auth.onAuthStateChange(function(event,session){rememberAuthSession(session);if(event==='PASSWORD_RECOVERY'){passwordRecoveryActive=true;setTimeout(function(){showAuth();setAuthMode('recovery');authMessage('Enter and confirm your new password.')},0);return}if(event==='SIGNED_OUT'){clearPrivateDeviceData();if(!currentUserProfile)showAuth()}});var result,session;try{result=await withTimeout(supa.auth.getSession(),10000,'The sign-in session could not be checked.');if(result.error)throw result.error;session=result.data&&result.data.session;if(session&&authSessionNeedsRefresh(session))try{session=await refreshAuthSession()}catch(refreshError){refreshError.startupSessionFailed=true;throw refreshError}}catch(error){if(error&&error.startupSessionFailed){clearPrivateDeviceData();currentUser=null;currentAccessToken='';showAuth('Your saved sign-in has expired. Sign in again to open the shared roster.',true)}else if(typeof showLaunchRecovery==='function')showLaunchRecovery('The saved sign-in did not respond. Check your connection, then try again.');else showAuth('Night Roster cannot connect right now. Check your internet connection and try again.',true);return}rememberAuthSession(session);if(session&&!passwordRecoveryActive)await authorizeUser(session.user,session);else showAuth()}
