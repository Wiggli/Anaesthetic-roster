/* Anaesthetic Night Roster V29.0 interface, staffing, allocation and PWA features. */
...
function showReleaseNotesIfNeeded(){
  ...
  if(seen===APP_VERSION)return;localStorage.setItem('anaes_seen_version',APP_VERSION);if(returning){var dialog=byId('releaseNotes');setTimeout(function(){if(dialog&&dialog.showModal&&!dialog.open)dialog.showModal()},900)}
}
...
function prepareChangesView(){
  ...
  byId('viewRosterBtn').onclick=function(){show('roster')};byId('closeRosterBtn').onclick=function(){show('today')};byId('smartNightBtn').onclick=goToAutomaticNight;byId('changesSmartNightBtn').onclick=goToAutomaticNight;byId('closeNamePickerBtn').onclick=function(){byId('personalNamePicker').classList.add('hidden')};continueButton.onclick=function(){setChangesStep('allocation',true)};confirmButton.onclick=function(){setChangesStep('confirm',true)};Array.prototype.forEach.call(document.querySelectorAll('[data-changes-step]'),function(button){button.onclick=function(){setChangesStep(button.getAttribute('data-changes-step'),true)}});var release=byId('releaseNotes');if(release){release.querySelector('.time').textContent='Version 29.0 is ready.';release.querySelector('ul').innerHTML='<li>Completely re-composed Night, Changes and Breaks screens</li><li>Personal allocation and outstanding decisions prioritised</li><li>Distinct high-contrast shift surfaces with one restrained design system</li>'}changesViewPrepared=true;renderDiagnostics();
}
...
function updateChangesWorkflow(base,plan){
  ...
  state.classList.toggle('complete',!confirmNeeded);
...
function renderBreaks(){
  ...
  var absenceCount=changesFor(base.date).length,summary=byId('breakSummaryRow');if(summary)summary.innerHTML='<span class="breakSummaryItem staffing"><b>'+esc(count)+'</b><small>Nurses</small></span><span class="breakSummaryItem '+(absenceCount?'absence':'ready')+'"><b>'+(absenceCount?esc(absenceCount):'No')+'</b><small>Absence'+(absenceCount===1?'':'s')+'</small></span>'+(labourPending?'<button type="button" class="breakSummaryItem labour pending" data-go-allocation><b>Labour Ward</b><small>Order pending ›</small></button>':'<span class="breakSummaryItem labour ready"><b>Labour Ward</b><small>Order complete</small></span>');
