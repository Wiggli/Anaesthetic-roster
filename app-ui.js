...
function renderChanges(base){
  ...
  var changeDetail={
    ...
    history:visible.map(function(h){return{label:h.label,type:h.type,title:h.title,detail:h.detail||'',meta:(h.changed_by||'Shift member')+' · '+shortTime(h.changed_at)}}),historyTotal:history.length,historyExpanded:expanded,allocations:allocationRows,allocationMessage:allocationMessage
  };
