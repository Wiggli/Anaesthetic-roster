/* Anaesthetic Night Roster V35.4 interface, staffing, allocation and PWA features. */
...
function updateChangesWorkflow(base,plan){
  ...
  var confirmButton=byId('continueToConfirmBtn');confirmButton.disabled=!!tasks;confirmButton.classList.toggle('hidden',!confirmNeeded);confirmButton.textContent=tasks?taskInstruction:'Review changes';var confirmReason=byId('continueToConfirmReason');if(confirmReason){confirmReason.textContent=tasks?'Unavailable until you '+taskInstruction.charAt(0).toLowerCase()+taskInstruction.slice(1)+'.':'';confirmReason.classList.toggle('hidden',!tasks)}
...
function subscribeToChanges(){
  ...
  changesChannel=supa.channel('roster-live-v35-4');
