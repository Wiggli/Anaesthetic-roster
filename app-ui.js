/* Anaesthetic Night Roster V34.2 interface, staffing, allocation and PWA features. */
...
var RELEASE_HISTORY=[
  {version:'34.2',date:'30 Aug 2026',title:'A clearer roster, personal account and safer sign-in',changes:['Night, Changes and Breaks now use one consistent Apple-inspired interface with clearer typography, restrained translucent navigation, native-style motion and matching light and dark appearances.','The update window now shows the complete changelog for the current version only, while earlier updates remain available separately from Account.','An ordinary six-nurse night stays quiet: Pager automatically works the Labour Ward first part with second break, while Reliever automatically works the second part with first break.','The Confirm stage now says Not needed instead of showing a misleading completion tick when the standard plan has not changed.','Five-nurse nights use one concise arrangement card without a duplicate warning, clearer full-night wording and more appropriate break guidance.','The agreed night-only role editor remains available for swapping any two core roles without changing the permanent rotation, and a short reason is still required.','The account sheet now includes an editable preferred name, optional role title, roster highlight, private profile photo, appearance settings, installation help, version history and the option to replay the introduction.','Saved photographs, preferred names and role titles personalise Your night, while approved roster names remain unchanged throughout the shared allocation plan.','The introduction now explains passkey privacy and includes optional profile setup for a preferred name, role title and photograph.','The Save profile button appears only after something has changed, and photo selection, cancellation and removal now provide clearer feedback.','Recent absence, overtime and agreed night-only role changes offer a temporary Undo action that reverses the shared change.','Supported devices can use an optional passkey with fingerprint, face recognition, device PIN or a security key, while password sign-in and password recovery remain available.','Allocation wording has been standardised across the dashboard, Changes, Breaks, full roster, exports and confirmations.','All existing roster calculations, staffing rules, navigation, live data, automatic Labour Ward logic and absence and overtime workflows remain preserved.']},
...
function onboardingPages(){
  ...
  return[
    ...
    '<div class="onboardingVisual identityVisual" aria-hidden="true"><span>'+(selected?esc(selected.charAt(0).toUpperCase()):'?')+'</span></div><span class="onboardingEyebrow">Your roster identity</span><h2 id="onboardingTitle">Which nurse are you?</h2><p>Choose your roster name so Night Roster can highlight your own allocation without changing the shared roster.</p><label class="onboardingNameLabel"><span>Roster name</span><select id="onboardingNamePick"><option value="">Choose your name</option>'+TEAM.map(function(name){return'<option value="'+esc(name)+'" '+(sameNurse(name,selected)?'selected':'')+'>'+esc(name)+'</option>'}).join('')+'</select></label><p class="onboardingFootnote">Your roster name stays separate from your preferred display name.</p>',
    '<div class="onboardingProfileSetup"><button type="button" class="onboardingProfilePhoto" id="onboardingPhotoBtn" aria-label="Choose profile photo">'+(avatar?'<img id="onboardingPhotoPreview" src="'+esc(avatar)+'" alt="">':'<span id="onboardingPhotoInitial">'+esc(profileInitial)+'</span>')+'<i aria-hidden="true">+</i></button><div><span class="onboardingEyebrow">Optional profile</span><h2 id="onboardingTitle">Make Your night yours.</h2><p>Add a preferred name, role title or photograph. These personalise your own dashboard without changing official roster names.</p></div></div><div class="onboardingProfileFields"><label><span>Preferred name</span><input id="onboardingProfileName" maxlength="60" autocomplete="name" value="'+esc(preferred)+'" placeholder="How the app greets you"></label><label><span>Role title <small>(optional)</small></span><input id="onboardingProfileTitle" maxlength="80" autocomplete="organization-title" value="'+esc(roleTitle)+'" placeholder="For example, Anaesthetic Nurse"></label></div><p class="onboardingFootnote">You can skip these details and add them later from Account.</p>'
  ];
}
...
async function finishOnboarding(){
  var select=byId('onboardingNamePick');if(select&&select.value)localStorage.setItem('anaes_my_name',select.value);var preferred=byId('onboardingProfileName'),title=byId('onboardingProfileTitle');if((preferred||title||pendingProfilePhoto)&&profileFeatureAvailable&&navigator.onLine){populateAccountSheet();if(preferred)byId('profileName').value=preferred.value.trim();if(title)byId('profileJobTitle').value=title.value.trim();updateProfileSaveState();if(!byId('saveProfileBtn').classList.contains('hidden'))await saveProfile()}
  localStorage.setItem('anaes_onboarding_complete_v34','1');onboardingCandidate=false;var dialog=byId('onboardingDialog');if(dialog&&dialog.open)dialog.close();render();toast('Your night is ready');
}
...
function bindOnboarding(){
  ...
  next.onclick=async function(){var last=onboardingPages().length-1;if(onboardingStep<last){onboardingStep++;renderOnboarding()}else{next.disabled=true;next.textContent='Saving…';await finishOnboarding();next.disabled=false}};back.onclick=function(){if(onboardingStep>0){onboardingStep--;renderOnboarding()}};skip.onclick=finishOnboarding;
}
...
function prepareChangesView(){
  ...
  var summaryTitle=document.createElement('div');summaryTitle.className='sectionHeadingRow nightSummaryTitle';summaryTitle.innerHTML='<h2 class="sectionTitle">Night summary</h2><span>Tap any item</span>';statusRow.id='nightStatusRow';statusRow.className='nightStatusRow';personal.insertAdjacentElement('afterend',summaryTitle);summaryTitle.insertAdjacentElement('afterend',statusRow);
...
function updateChangesWorkflow(base,plan){
  ...
  var staffingTab=document.querySelector('[data-changes-step="staffing"]'),allocationTab=document.querySelector('[data-changes-step="allocation"]'),confirmTab=document.querySelector('[data-changes-step="confirm"]'),confirmationNotNeeded=!hasManualPlan&&!confirmNeeded;staffingTab.classList.add('complete');allocationTab.classList.toggle('complete',!tasks);allocationTab.classList.toggle('hasTasks',!!tasks);confirmTab.classList.toggle('complete',!confirmNeeded&&!confirmationNotNeeded);confirmTab.classList.toggle('notNeeded',confirmationNotNeeded);confirmTab.classList.toggle('hasTasks',!!confirmNeeded);
...
function breakData(r){
  ...
  if(r.mode==='5')notes.push(r.fullLW+' covers Labour Ward / Pager for the full night. Their break is coordinated during the shift when clinical cover allows.');
  else{
    var order=labourOrderFor(r);
    if(order){
    ...
    if(r.mode==='7')notes.push(r.seventh+' is the seventh nurse and coordinates a break as required.');
...
function renderRoster(){
  ...
  R.forEach(function(original,i){
    ...
    else if(r.mode==='5')html+='<div class="row"><div class="lab">Full-night Labour Ward / Pager</div><div><span class="tag tFull">'+esc(r.fullLW)+'</span></div></div>';
    ...
    if(r.mode==='7')html+='<div class="row"><div class="lab">Seventh nurse</div><div><span class="tag t7">'+esc(r.seventh)+'</span></div></div>';
