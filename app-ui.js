/* Anaesthetic Night Roster V37.38 interface, staffing, allocation and PWA features. */
...
function installGuideSteps(){
  ...
  return'<div class="installGuideHero"><img src="icon-192.png?v=37.38" alt=""><div><b>'+esc(standalone?'Installed':'Night Roster')+'</b><span>'+esc(label)+'</span></div></div><div class="installSteps">'+steps.map(function(step,index){return'<div class="installStep"><b>'+(index+1)+'</b><span>'+esc(step)+'</span></div>'}).join('')+'</div><p class="installGuideFootnote">No App Store or Play Store account is required. Shared roster data stays in Supabase and existing sign-in continues to work.</p>';
}
