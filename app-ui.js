/* Anaesthetic Night Roster V37.16 interface, staffing, allocation and PWA features. */
...
var RELEASE_HISTORY=[
  {version:'37.16',date:'24 Sep 2026',title:'Google and Microsoft sign-in ready',changes:['The sign-in screen now supports Google and Microsoft alongside the existing email, password and passkey options.','OAuth sign-in returns to the installed Night Roster and still passes through the same approved-email and RLS checks before any roster data can open.','Microsoft requests only the email scope required for roster identity checks, and provider secrets remain outside the browser code.','Existing roster calculations, staffing, allocations, Pager, Reliever and database permissions remain unchanged.']},
...
function bind(){
  ...
  byId('loginTab').onclick=function(){setAuthMode('login')};byId('signupTab').onclick=function(){setAuthMode('signup')};byId('authSubmitBtn').onclick=submitAuth;byId('authGoogleBtn').onclick=function(){signInWithSocialProvider('google')};byId('authMicrosoftBtn').onclick=function(){signInWithSocialProvider('azure')};byId('authPasskeyBtn').onclick=signInWithPasskey;byId('authPasskeyBtn').classList.toggle('hidden',!passkeySupported());byId('forgotPasswordBtn').onclick=requestPasswordReset;byId('cancelRecoveryBtn').onclick=function(){setAuthMode('login')};byId('authPassword').onkeydown=function(e){if(e.key==='Enter')submitAuth()};byId('authPasswordConfirm').onkeydown=function(e){if(e.key==='Enter')submitAuth()};
