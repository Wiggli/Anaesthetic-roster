/* Anaesthetic Night Roster V37.48 interface, staffing, allocation and PWA features. */
...
function installGuideSteps(){
  ...
  return'<div class="installGuideHero"><img src="icon-192.png?v=37.48" alt=""><div><b>'+esc(standalone?'Installed':'Night Roster')+'</b><span>'+esc(label)+'</span></div></div><div class="installSteps">'+steps.map(function(step,index){return'<div class="installStep"><b>'+(index+1)+'</b><span>'+esc(step)+'</span></div>'}).join('')+'</div><p class="installGuideFootnote">No App Store or Play Store account is required. Shared roster data stays in Supabase and existing sign-in continues to work.</p>';
}
...
async function runAccountAction(action){
  ...
  if(action==='versions'){byId('accountSheet').close();renderReleaseNotes(true);byId('releaseNotes').showModal();return}
  if(action==='install'){byId('accountSheet').close();if(deferredInstallPrompt){deferredInstallPrompt.prompt();await deferredInstallPrompt.userChoice;deferredInstallPrompt=null;byId('installBtn').classList.add('hidden')}else showInstallGuide()}
}
...
function renderReleaseNotes(showHistory){
  ...
  if(!window.__forceReleaseNotesFallback&&typeof window.renderReactReleaseNotes==='function'){window.renderReactReleaseNotes(entries,!!showHistory);return}
  var host=byId('releaseNotesContent');if(!host)return;host.innerHTML='<div class="releaseNav'+(showHistory?'':' hidden')+'" aria-label="Version history navigation">'+(showHistory?'<button type="button" data-release-jump="latest">Latest</button><button type="button" data-release-jump="previous">Previous updates <span>'+esc(RELEASE_HISTORY.length-1)+'</span></button>':'')+'</div><div class="releaseHistory" tabindex="0" aria-label="'+esc(showHistory?'Complete Night Roster version history':'Changes in version '+latest.version)+'">'+entries.map(function(entry,index){var archive=index===1?'<div class="releaseArchiveHeading"><span>Previous updates</span><small>The work that shaped Night Roster</small></div>':'';return archive+'<section class="releaseEntry '+(index===0?'latest':'')+'"><div class="releaseVersion"><div><span>'+(index===0?'Current update · ':'')+'Version '+esc(entry.version)+'</span><h3>'+esc(entry.title)+'</h3></div><time>'+esc(entry.date)+'</time></div><ul>'+entry.changes.map(function(change){return'<li>'+esc(change)+'</li>'}).join('')+'</ul></section>'}).join('')+'</div>';
  var history=host.querySelector('.releaseHistory'),nav=host.querySelector('.releaseNav');history.scrollTop=0;Array.prototype.forEach.call(nav.querySelectorAll('[data-release-jump]'),function(button){button.onclick=function(){var previous=button.getAttribute('data-release-jump')==='previous',target=previous?history.querySelector('.releaseArchiveHeading'):history.querySelector('.releaseEntry.latest');if(!target)return;history.scrollTo({top:Math.max(0,target.offsetTop-(previous?0:8)),behavior:window.matchMedia&&window.matchMedia('(prefers-reduced-motion: reduce)').matches?'auto':'smooth'})}});
  if(typeof window.CustomEvent==='function')window.dispatchEvent(new CustomEvent('roster:releasenotes',{detail:{entries:entries,showHistory:!!showHistory}}));
}
...
function render(){
  ...
  var decisionTasks=workflowTaskCount(base,plan,r),confirmNeeded=workflowNeedsConfirmation(base,decisionTasks),taskCount=decisionTasks||(confirmNeeded?1:0),absenceCount=changesFor(base.date).length,overtimeCount=overtimeFor(base.date).length;
...
function updateAllocationSaveControl(base,plan){
  plan=plan||staffingPlan(base);var hasAllocationChoices=overtimeFor(base.date).length&&plan.availableKeys.length,button=byId('saveAllocationsBtn');
...
function bind(){
  ...
  byId('closeScreenInfoSheet').onclick=function(){byId('screenInfoSheet').close()};byId('closeActivityDetailSheet').onclick=function(){byId('activityDetailSheet').close()};Array.prototype.forEach.call(document.querySelectorAll('.bottom button'),function(b){b.onclick=function(){var view=b.getAttribute('data-v');show(view);if(view==='chat'&&typeof window.openChatView==='function')window.openChatView()}});updateOfflineControls();
}
