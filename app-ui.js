...
var RELEASE_HISTORY=[
  {version:'37.42',date:'25 Sep 2026',title:'Swipes stay responsive between screens',changes:['A new swipe can now begin immediately while the previous page is finishing its short settle animation. The app completes the pending destination first instead of silently dropping the new touch.','Rapid Night, Changes, Breaks and Chat navigation therefore stays responsive even when gestures follow each other closely. Continuous bottom-bar dragging and the one-time horizontal direction lock introduced in 37.41 remain unchanged.','External tab changes also cancel any obsolete settling state before a new gesture starts, preventing an old pending screen from taking over a newer selection.','Roster calculations, Supabase data, authentication, protected controls and the safe PWA update flow are unchanged.']},
