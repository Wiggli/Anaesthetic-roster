...
var RELEASE_HISTORY=[
  {version:'37.16',date:'24 Sep 2026',title:'Google and Microsoft sign-in',changes:['Approved roster members can now start sign-in with Google or Microsoft from the existing account screen.','OAuth returns to the same Night Roster address and still passes through the existing allowed-users and RLS authorization checks before roster data opens.','Microsoft sign-in requests only the email scope needed for roster authorization, while provider credentials remain in Supabase rather than GitHub or browser code.','Email/password, password reset and passkey sign-in remain available, with no change to roster calculations, staffing, allocations, Pager or Reliever.']},
...
function bind(){
  ...
  byId('loginTab').onclick=function(){setAuthMode('login')};byId('signupTab').onclick=function(){setAuthMode('signup')};byId('authSubmitBtn').onclick=submitAuth;byId('authGoogleBtn').onclick=function(){signInWithOAuthProvider('google')};byId('authMicrosoftBtn').onclick=function(){signInWithOAuthProvider('azure')};byId('authPasskeyBtn').onclick=signInWithPasskey;byId('authPasskeyBtn').classList.toggle('hidden',!passkeySupported());byId('forgotPasswordBtn').onclick=requestPasswordReset;byId('cancelRecoveryBtn').onclick=function(){setAuthMode('login')};byId('authPassword').onkeydown=function(e){if(e.key==='Enter')submitAuth()};byId('authPasswordConfirm').onkeydown=function(e){if(e.key==='Enter')submitAuth()};
