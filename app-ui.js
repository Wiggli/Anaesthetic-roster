/* Anaesthetic Night Roster V37.42 interface, staffing, allocation and PWA features. */
...
var RELEASE_HISTORY=[
  {version:'37.42',date:'25 Sep 2026',title:'Reliable swiping everywhere',changes:['Horizontal swipes now start from ordinary cards, summary tiles and the Chat transcript instead of being silently ignored because the finger landed on a non-button container. Actual buttons, links, inputs, composers and dialogs remain protected.','The bottom tab lens now follows the finger position directly across the whole bar, so a long drag stays continuous instead of depending on the tab where the gesture began.','Page swipes keep the horizontal direction lock introduced in 37.41, which prevents small diagonal movement from cancelling the gesture midway. Night and Chat are specifically covered by the mobile regression tests.','This release also repairs the failed 37.41 production verification before deployment. Roster calculations, Supabase data, authentication and clinical workflow rules are unchanged.']},
...
function installGuideSteps(){
  ...
  return'<div class="installGuideHero"><img src="icon-192.png?v=37.42" alt=""><div><b>'+esc(standalone?'Installed':'Night Roster')+'</b><span>'+esc(label)+'</span></div></div><div class="installSteps">'+steps.map(function(step,index){return'<div class="installStep"><b>'+(index+1)+'</b><span>'+esc(step)+'</span></div>'}).join('')+'</div><p class="installGuideFootnote">No App Store or Play Store account is required. Shared roster data stays in Supabase and existing sign-in continues to work.</p>';
}
