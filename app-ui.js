/* Anaesthetic Night Roster V37.46 interface, staffing, allocation and PWA features. */
...
function installGuideSteps(){
  ...
  return'<div class="installGuideHero"><img src="icon-192.png?v=37.46" alt=""><div><b>'+esc(standalone?'Installed':'Night Roster')+'</b><span>'+esc(label)+'</span></div></div><div class="installSteps">'+steps.map(function(step,index){return'<div class="installStep"><b>'+(index+1)+'</b><span>'+esc(step)+'</span></div>'}).join('')+'</div><p class="installGuideFootnote">No App Store or Play Store account is required. Shared roster data stays in Supabase and existing sign-in continues to work.</p>';
}
...
function bind(){
  ...
  window.addEventListener('roster:account-action',function(event){var detail=event&&event.detail||{};if(detail.action==='theme')setThemePreference(detail.value);else if(detail.action==='passkey-remove')deletePasskey(detail.value);else runAccountAction(detail.action)});
  ...
  var legacySaveChange=byId('saveChangeBtn'),legacyCancelAbsence=byId('cancelAbsenceEditBtn'),legacyAbsent=byId('absentName'),legacyAddOvertime=byId('addOvertimeBtn'),legacySaveAllocations=byId('saveAllocationsBtn'),legacyOvertime=byId('overtimeName');if(legacySaveChange)legacySaveChange.onclick=saveNightChange;if(legacyCancelAbsence)legacyCancelAbsence.onclick=cancelAbsenceEdit;if(legacyAbsent)legacyAbsent.onchange=function(){markInvalid('absentName',false);formMessage('absenceFormMessage','');updateStaffingActionAvailability()};if(legacyAddOvertime)legacyAddOvertime.onclick=saveOvertime;if(legacySaveAllocations)legacySaveAllocations.onclick=saveFinalAllocationsV2510;if(legacyOvertime){legacyOvertime.oninput=function(){markInvalid('overtimeName',false);formMessage('overtimeFormMessage','');updateStaffingActionAvailability()};legacyOvertime.onkeydown=function(e){if(e.key==='Enter'&&!byId('addOvertimeBtn').disabled)saveOvertime()}}byId('addAccountBtn').onclick=addAuthorisedAccount;
